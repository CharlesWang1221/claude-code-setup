...
| --- | --- | --- | --- | --- |
| 新集數從素材到跨平台發布 | 內容營運長 | `podcast-publish`（星期天） | 集數、音檔、逐字稿／時間碼、公開範圍 | 先讀品牌與各當下 Skill；未 `ALLOW`／無平台證據，不得發布；IG 圖、YouTube 正片、預告與片尾都由同一集狀態檔追蹤 |
...
| Podcast 單集 IG 輪播／圖卡 | 內容營運長＋品牌總監 | `podcast-publish` 的 IG 圖步驟 | 已放行文案、封面與比例規格 | 不存在獨立 `social-cards` Skill；不得另派不存在的 Skill |
