...
- 模式 B 的獨立品牌網誌，預設排在週四 10:00（Asia/Taipei, UTC+8）發布；先採每月 2 篇，不把週更當 KPI。
- 文章必須同時具備明確品牌立場與真實案例。若只是為補 SEO 數量，不進入發布排程。
- 網站文章於週四 10:00 發布後，同篇 FB 長文與啊問核准主圖預設排在週四 20:30，讓網站與社群錯峰。
- 週五 12:00 是共用「編輯席位」：從 Podcast 金句、節目觀點或其他當週內容中擇一。週四已發的網誌不再重複導讀。
- 本 Skill 完成草稿與驗證後，仍停在人工核准關卡。老查明確要求上線時，才依最近一個核准的週四 10:00 安排 commit、push 與 deploy；若要改日期，先確認，不自行提前公開。
- 此節奏由居易管理，不併入「星期天」的 Podcast 上架狀態表。
...
文章定稿後，呼叫 `blog-image-director`（老查取名「啊問」）讀取完整文章並建議最合適的網誌圖片呈現方式。啊問只提出首選與最多一個備選；老查確認後才製作圖片。若老查已指定「分隔漫畫」等現有視覺模組，直接依該模組執行。

網站稿與圖片都核准後，若老查要求 Facebook 上架，將文章改成可直接閱讀的 FB 長文排版，文末加 4 至 6 個精準 hashtag，並預設排程週四 20:30（Asia/Taipei）。Facebook 是否排程成功必須以平台顯示的日期與時間為準，不得把已填表誤報為已排程。
...
同時把同一份文章複製一份到 Google Drive。模式 A、模式 B 一律用文章的中文標題建立資料夾，不再用 episode slug 分類：

`G:\我的雲端硬碟\不標準答案\2026\網誌\未發布\{文章title}\{article-slug}.md`

資料夾名稱必須使用文章 frontmatter 的完整中文 `title`，方便老查直接辨識內容。真正的 episode slug 仍保留在網站文章檔名或發布狀態，不拿來命名 Drive 資料夾。
