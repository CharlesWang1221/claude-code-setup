...
- 無論使用 Mac 或 Windows PC，主要工作代理固定是 Codex；Claude Cloud／Claude Code 僅作輔助。不要把 Claude 的規則、Skills 或 MCP 設定反向覆蓋 Codex。
...
- 目前主力工作機為 Apple Silicon Mac（2026-08-22）。Codex 是主要代理；Claude Code 只保留 `playwright`、`firecrawl`、`cloudflare`、`google-workspace`、`plaud` 作輔助。`filesystem` 已從兩邊移除（跟內建檔案工具重複）。Codex 端 `~/.codex/config.toml` 含 API key，已加進 `.gitignore`，不要移除該規則。
